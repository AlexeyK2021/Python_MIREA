    ...
    for i in range(len(data)):
        for j in range(len(data[i])):
            if data[i][j] is not None and ":" in data[i][j]:
                new_data[i][j] = data[i][j][data[i][j].find(":") + 1:]
                t = data[i][j].rfind("-")
                data[i][j] = (data[i][j][3:t] + data[i][j][t + 1:data[i][j].find(":")]).replace(' ', '-')
...
if __name__ == "__main__":
    print(*main([['+7 978 692-67-33:0.8', '21/10/01', None, 'fadanz5@mail.ru', 'fadanz5@mail.ru'],
                 ...
                 ['+7 516 438-25-95:0.8', '05/05/03', None, 'fesodak55@yandex.ru', 'fesodak55@yandex.ru']]), sep="\n")
